...
import { useProductView, useGallery, useRelatedProducts } from '../../hooks/useProductDetailLogic.js';
...
const ACCORDION_ITEMS = [
    {
        ...
        getContent: (product) => product?.description || "Product description is loading..."
    },
    {
        id: 'sizeAndFit',
        title: 'Size & Fit',
...
const ProductDetail = memo(() => {
    ...
    // Derived Logic
    const colorCount = useMemo(() => new Set(siblingVariants.map(v => v.color)).size, [siblingVariants]);
    ...
    const { sameColorVariants, needsSizeSelection } = useMemo(() => {
        const sameColor = siblingVariants.filter(v => v.color === currentColorName);
        const sizes = [...new Set(sameColor.map(v => v.size))].filter(Boolean);
        const isFree = sizes.length === 1 && sizes[0]?.toLowerCase() === 'free size';
    ...
    const handleAddToBagFreeSize = useCallback(() => {
        ...
        addToCartMutation.mutate(
            { variantId: variant._id, quantity: 1 },
            {
                ...
                onError: (error) => alert(error.response?.data?.message || 'Failed to add to bag. Please login first.')
            }
        );
