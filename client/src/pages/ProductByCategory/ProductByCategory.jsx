...
const ProductByCategory = () => {
    ...
    // Check xem category có thumbnail không
    const hasThumbnail = category?.thumbnailUrl;

    // 4 variants đầu tiên cho leftBox (chỉ khi có thumbnail)
    const firstFourVariants = hasThumbnail ? variants.slice(0, 4) : [];

    // Các variants còn lại cho productList
    const remainingVariants = hasThumbnail ? variants.slice(4) : variants;
    ...
    return (
        <div className={styles.productByCategory}>
            ...
            <div className={styles.countAndFilter} style={{ top: `${headerHeight}px` }}>
                <span className={styles.count}>{variants.length} items</span>
