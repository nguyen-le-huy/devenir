...
// TODO: Define strict Order type
// Using any for now to facilitate migration
type Order = any;
...
// fix implicit any
const formatDate = (v: any) => (v ? format(new Date(v), 'MM/dd HH:mm', { locale: enUS }) : '—')
...
function DetailPanel({ order, showTimeline = false }: { order: Order; showTimeline?: boolean }) {
    if (!order) {
        return (
    ...
    const totalItems = order.orderItems?.reduce((s: number, i: any) => s + (i.quantity || 0), 0) || 0
...
function ShipStatusStrip({ order }: { order: Order }) {
        ...
        <div className={styles.shipStrip}>
            <div className={styles.stripHeader}>
                ...
                <div className={styles.stripMeta}>
                    ...
                    <span>Items: {order.orderItems?.reduce((s: number, i: any) => s + (i.quantity || 0), 0) || 0}</span>
                </div>
            </div>
...
export default function ProfileOrders() {
    ...
    // useMyOrders is likely not typed yet or returns any
    const { data, isLoading, error } = useMyOrders() as any

    // Handle both structures: { data: [...] } or direct array
    ...
    const filtered = useMemo(() => {
        ...
        list.sort((a, b) => (sortMode === 'newest' ? new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime() : new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()))
        return list
    ...
    useEffect(() => {
        ...
        const handleOrderUpdate = (payload: any) => {
            queryClient.setQueriesData({ queryKey: queryKeys.orders.all }, (existing: any) => {
