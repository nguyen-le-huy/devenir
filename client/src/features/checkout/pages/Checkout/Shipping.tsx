...
const Shipping = memo(() => {
    ...
    useEffect(() => {
        if ((addressData as any)?.data) {
            setSavedAddress({ ...(addressData as any).data });
            setFormData((addressData as any).data);
        }
    }, [addressData]);
    ...
    const handlePayWithPayOS = async () => {
        ...
        try {
            ...
            // Try both structures: nested (response.data.checkoutUrl) or root level (response.checkoutUrl)
            const checkoutUrl = response?.data?.checkoutUrl || response?.checkoutUrl;

            if ((response?.success && checkoutUrl) || checkoutUrl) {
