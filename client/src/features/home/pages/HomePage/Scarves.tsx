...
const Scarves = () => {
    ...
    const { scarvesCategory, allCategoryIds } = useMemo(() => {
        ...
        const allIds = [parentCategory._id, ...subcategories.map((sub: any) => sub._id)];
    ...
    const { data: allVariants, isLoading: isVariantsLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            // @ts-ignore
            const { getVariantsByCategoryWithChildren } = await import('@/features/products/api/productService.js');
    ...
    const scarves = useMemo(() => {
        ...
        return allVariants.slice(0, 12).map((variant: any) => ({
            id: variant._id,
