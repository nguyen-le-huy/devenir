...
const formatDate = (v) => (v ? format(new Date(v), 'dd/MM HH:mm', { locale: vi }) : '—')
  ...
  if (!order) {
    return (
      <div className={styles.detailPlaceholder}>
        <div className={styles.placeholderTitle}>Chọn một đơn để xem tiến trình giao hàng</div>
        <p className={styles.subtle}>Chi tiết và trạng thái sẽ hiển thị tại đây.</p>
      </div>
    )
  }
  ...
  return (
    <div className={styles.detailCard}>
      ...
      <div className={styles.detailMeta}>
        ...
        <div>
          <div className={styles.metaLabel}>Sản phẩm</div>
          <div className={styles.metaValue}>{totalItems} món</div>
        </div>
      </div>
    ...
    <div className={styles.shipStrip}>
      <div className={styles.stripHeader}>
        ...
        <div className={styles.stripMeta}>
          <span>Mã vận đơn: {order.trackingNumber || 'Đang tạo'}</span>
          <span>Sản phẩm: {order.orderItems?.reduce((s, i) => s + (i.quantity || 0), 0) || 0}</span>
        </div>
      </div>
    ...
    <div className={styles.container}>
      <div className={styles.header}>
        <div>
          ...
          <p className={styles.note}>Trang sẽ tự làm mới mỗi 10 giây khi có đơn đang giao.</p>
        </div>
      </div>
      ...
      {!isLoading && !orders.length && !error && (
        <div className={styles.empty}>
          ...
          <div className={styles.subtle}>Bạn sẽ thấy lịch sử tại đây sau khi thanh toán thành công.</div>
        </div>
      )}
                  ...
                  {filtered.map((order) => (
                    <tr key={order._id} onClick={() => setSelectedId(order._id)} className={selectedOrder?._id === order._id ? styles.rowActive : ''}>
                      ...
                      <td className={styles.cellMain}>
                        <div className={styles.itemName}>{order.orderItems?.[0]?.name || 'Đơn hàng'}</div>
                        <div className={styles.subtle}>{order.orderItems?.length || 0} sản phẩm</div>
                      </td>
                      <td>
